...
module.exports = {
  async index(req, res) {
    try {
      ...
      // Ja realizados no SIENGE: entram no custo/receita da obra, mas nunca
      // geram titulo ou movimento bancario adicional no Fluxy.
      ...
      for (const row of historicos) {
        ...
        const atual = mapAgregados[obraId]?.[tipo] || {
          total_valor_original: 0, total_valor_baixado: 0,
          total_valor_saldo: 0, quantidade: 0
        };
